...
Data sources For this project, necessary data were collected from the following 3 sources:

1.Github repository (Daily covid morbidity)(https://github.com/nytimes/covid-19-data)
...
The process flow for this project explains it in a small scale:
...
As we are dealing with the New York state county data only and the maximum distance was less than 400 miles from the epicentre of the COVID-19 disease, which is New York City. Here for the sake of the analysis only New York county that is Manhattan city was considered as the centre. So, the file which contained counties within 500 miles distance was our target file. The required CSV file (2010 census data) was directy downloaded from https://www.nber.org/research/data/county-distance-database by clicking on the link given in the webportal (Link url: https://data.nber.org/distance/2010/sf1/county/sf12010countydistancemiles.csv.zip )

zip file was downloaded thereby and when extracted, it revealed the following csv: sf12010countydistancemiles.csv

The extracted file was renamed as 'distance_data.csv' and it was moved to the directory that contains this notebook. Overviewing the downloaded distance data with python, it was observed that distance among each of the counties of the USA are given here; hence the file is large. So, I decided to make a smaller file by filtering some data (to keep only New York state counties). Following table shows some data as an example.
...
2a. Filtering the morbidity data and creating a new file with the filtered data The file contained all the county data of the United States. So, data was filtered and a new csv file called covid_data_filtered.csv was created which contained only New York state county data. Here are some of the data.
...
It was observed that the all fips code of NY state counties started with '36'which is the state code. checking the filtered data file further subsection was done based on state code '36' and the file was named as distance_data_filtered.csv.
...
2c. Checking the NY state county names 'collected' by the web scraping
New York State has 62 counties and by web scraping got the county names.
...
3.Transferring data to a relational database
...
The following csv files were tansfered to a MySQL database (hosted by Clarkson MSDA)

'distance_data_filtered.csv'
'covid_data_filtered.csv'
'population_data.csv'
...
3b.Following steps were followed to build the databaseDatabase_stages.png

                                Figure: Steps to follow in database

i.Establishing Connection with clarkson database
ii.Creating tables in database
iii. Inserting data in nycovidcase table
iv. Inserting data in nydistance table
v. Inserting data in nypopulation table
...
4b. Inserting rows into nycovidcase by utilizing 'block size' concept Although the csv files here are not too large,still using this concept of sending data in chunks for the sake of a'good practice' so that the codes remain scalable for large data sets.

block size: 4000 - total time : 1.1710128784179688

database%20table.png
...
4d. Inserting rows into nycdistance block size: 400 - total time : 0.5940568447113037
...
For answering the the research question (Is there any relation between COVID-19 infection rate with different New York State counties and their distance from New York City?), We need to join the following three tables:
...
From table 1 : the number of total case
From table 2 : the distance of each county from Manhattan, NYC
From table 3 : population of each county (to determine the Rate of infection among the population in each county)


a.Connecting clarkson database
b.Checking population data

Making a function for executing sql commands
Fetches(from a connected database) the result of the given sql command which should be written in between tripple quotes. Establishing the database connection and cursor as 'cur' is a prerequisite
...
We wanted to know whether there is any relation between the COVID infection rate among the population of the NY state counties and their distance from the New York City. We collected the required data, transfered them to a database, made a new table with the required fields and imported back the table as a CSV. Now we can do the 'analytic' operations to findout the answer. Checking COVID case data
...
Exporting the content of the view table named rate_vs_distance.csv as a csv The content of the view table will now be exported to "Infection_Rate_Vs_Distance_2020_11_13.csv" for further operations.
...
correlation.pngText(0,0.5,'COVID Infection Percentage *10,000')

                            Figure:Scatter plot showing correlation of COVID-19 infection rate and distance from NYC
Just by observing the scatterplot, we can visually confirm that there is a negetive correlation between the distance of a county from Manhattan and its COVID-19 infection rate. So, in the next section, we'd go for checking it's statistical significance.

Finding the Pearson's coefficient and testing its statistical significance at a 95% level confidence. The python package 'scipi' is used to find the Pearson correlation coefficient We can test the significance with different confidence level, here the test with 95% confidence.
...
The result shows stat=-0.490, p=0.000 The coorelation is statistically significant at a 95% confidence level
...
The answer of the researsch question is 'Yes', meaning that there is a mooderate statistically significant relation beteen the COVID-19 infection rate among the population in NY state counties and their distances from New York City (Manhattan) at a significance level of 95%.

The less the the distance of a county from New York city, the more is the COVID-19 infection rate among its population.
